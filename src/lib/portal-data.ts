...
import { readLocalPortalStore } from "@/lib/local-portal-store";
...
export async function getPortalDataset(): Promise<PortalDataset> {
  const localStore = await readLocalPortalStore();

  if (!isSupabaseConfigured) {
    return {
      ...hydrateDataset(),
      clients: localStore.clients,
      clientIntegrations: localStore.clientIntegrations,
    };
  }
  ...
  const clients = clientsRes.error
    ? localStore.clients
  ...
  const integrations = integrationsRes.error
    ? localStore.clientIntegrations
    : ((integrationsRes.data ?? []) as Array<Record<string, unknown>>).map((integration) => ({
  ...
  const campaigns = campaignsRes.error
    ? demoDataset.campaigns
    : ((campaignsRes.data ?? []) as Array<Record<string, unknown>>).map((campaign) => ({
  ...
  const meetings = meetingsRes.error
    ? demoDataset.meetings
    : ((meetingsRes.data ?? []) as Array<Record<string, unknown>>).map((meeting) => ({
  ...
  const accounts = accountsRes.error
    ? demoDataset.accounts
    : ((accountsRes.data ?? []) as Array<Record<string, unknown>>).map((account) => ({
